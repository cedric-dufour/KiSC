...
class KiscCli_resource_start(KiscCli_kisc):
    """
    KiSC command-line utility - (Sub)command 'resource start'
    """
    ...
    def execute(self, _sCommand=None, _lArguments=None):
        ...
        try:
            ...
            if self._oArguments.bootstrap and not self._oArguments.force:
                raise RuntimeError('Can not start bootstrap (host startup) resource without \'--force\' flag')
            ...
            # Start resource
            lsErrors = oClusterResource.start(self._oArguments.force)
            if lsErrors:
                if self._oArguments.verbose >= KiscRuntime.VERBOSE_DEBUG:
                    for sError in lsErrors:
                        sys.stderr.write('%s\n' % sError)

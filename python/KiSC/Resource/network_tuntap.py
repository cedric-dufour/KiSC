...
class KiscResource_network_tuntap(KiscResource):
    """
    TUN/TAP (tunnel) network device resource

    Configuration parameters are:
     - [REQUIRED] name (STRING):
       tunnel name
     - [REQUIRED] mode (tun|tap):
       tunnel mode
     - [OPTIONAL] user (UID|USERNAME):
       owner user
     - [OPTIONAL] group (GID|GROUPNAME):
       owner group
    ...
    def stop(self):
        ...
        # ... delete device
        try:
            KiscRuntime.shell(['ip', 'tuntap', 'delete', 'dev', self._dsConfig['name'], 'mode', self._dsConfig['mode']], _bTrace = self._iVerbose >= KiscRuntime.VERBOSE_TRACE)

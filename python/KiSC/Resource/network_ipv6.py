...
class KiscResource_network_ipv6(KiscResource):
    """
    IPv6 network resource

    Configuration parameters are:
     - [REQUIRED] address (STRING; colon-separated hexadecimal IPv6):
       IPv6 network address
     - [REQUIRED] mask (NUMBER):
       network mask/prefix length
     - [REQUIRED] device (STRING):
       network device (interface) name
     - [OPTIONAL] label (STRING):
       address label
     - [OPTIONAL] scope (*global|site|link|host|NUMBER):
       address scope
    ...
    def status(self, _bStateful = True, _iIntent = None):
        ...
        if _bStateful:
            ...
            try:
                KiscRuntime.shell([
                    [ 'ip', '-6', 'address', 'show' ],
                    [ 'grep', '-Fqi', 'inet6 %s/' % self._dsConfig['address'].replace('::', ':0:') ]
                ], _bTrace = self._iVerbose >= KiscRuntime.VERBOSE_TRACE)
            except OSError as e:
